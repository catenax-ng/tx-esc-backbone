	...
	return sdk.NewDecWithPrec(5, 1).
		...
		Sub(
			sdk.NewDec(1).Quo(sdk.NewDec(6)).
				...
				Mul(c.S3.firstDerivativeT1(sdk.NewDec(0)).
					Quo(c.S3.DeltaX))).
		Add(sdk.NewDecWithPrec(5, 1).Mul(c.pY(p2)))
}

func (c *Curve) dx1() sdk.Dec {
	return sdk.NewDec(1).Sub(c.FactorFxy).Mul(c.pY(p2)).
		Sub(sdk.NewDec(1).Sub(c.FactorFxy).Mul(c.pY(p0)))
}
